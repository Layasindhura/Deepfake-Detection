...
def predict_page(request):
    if request.method == "GET":
        ...
        model_accuracy = model_file.split("_")[1]   # extract accuracy from filename
        ...
        print("<=== | Started Face Cropping Each Frame | ===> "+str(sequence_length+1))
        padding = 40
        faces_found = 0
        for i in range(1, sequence_length+1):
            frame = frames[i]
        ...
        try:
            ...
            return render(request, predict_template_name, {'preprocessed_images': preprocessed_images, 'heatmap_images': heatmap_images, "faces_cropped_images": faces_cropped_images, "original_video": video_file_name, "models_location": models_location, "output": output, "confidence": confidence, "model_accuracy": model_accuracy})
        except:
...
def login(request):
    if request.method == "POST":
        ...
        try:
            ...
            if user.password == password:  # In production, use hashing!
                # Login success
...
def image_detection(request):
    ...
    if request.method == "POST":
        ...
        model_path = os.path.join(settings.PROJECT_DIR, 'models', model_file)

        # Extract accuracy from filename
        model_accuracy = float(model_file.split("_")[1])
